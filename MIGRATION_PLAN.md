...
- One more trap worth knowing: Pages caches `index.html`, so a deploy can be tested against a **stale bundle**. Check the bundle hash the page actually loaded against `curl`ing the live HTML before believing a browser test — it produced one false failure here, and it also explains a real one: a re-upload made ~10 minutes *after* the duration fix deployed still stored `NULL`, because the browser was still running the pre-fix bundle. Verified by uploading fresh with a cache-busted load: MP3 `0:07`, MP4 `0:04`, PNG `null`.
