...
- [ ] ~~Visibility model (`PRIVATE/PUBLIC/UNLISTED` + `publicId`) — keep~~ **DROPPED**: a single admin has no public surface. Columns + the `visibility` enum were removed in migration `0001`; social engagement counters went with them.
  - **Revisited later: the archive became public to read** (Phase 6). The premise above — "no public surface" — stopped being true, but the *per-item* model did not come back: everything in the archive is readable, so there is still nothing for a `visibility` column to express. Per-item privacy would be a new feature with real work behind it (a column, a filter in every read query, a control to manage it), not a column to re-add on spec. Engagement counters remain dropped — nothing reads them.
...
- [x] **Selectable transcription provider** (Settings → Transcription). `app_settings` (migration `0004`) holds one row — provider (`GROQ` | `GOOGLE`) and model id — read once per queue batch, so a change takes effect on the next job with no deploy. `src/services/transcription/` is a provider abstraction (`types.ts` contract + error classification, `groq.ts`, `google.ts`, `index.ts` dispatcher) and nothing downstream branches on which one ran.
  - Groq stays the default because it is the only one returning **decoded** timestamps, and lyric sync, `?t=` deep links and transcript search all rest on those numbers. Gemini's are estimates — the same unreliability that had the old pipeline replaced — so choosing it is a quality trade, stated in the UI rather than hidden.
  - Gemini cannot fetch a URL, so the Worker reads the object from R2 and inlines base64 → a 14 MB cap and a narrower container allowlist. Both refusals are `permanent` errors that name the limit or the supported types, so they fail fast with a readable message instead of retrying into the DLQ.
  - Supersedes `GROQ_MODEL`: the model id is now a setting, and `GEMINI_API_KEY` joins `GROQ_API_KEY` as an optional secret. `GET /api/settings` also reports which providers actually have a key, so the UI can warn before a job fails rather than after.
...
- [x] **Provider-level tests** — `test/transcription.test.ts` drives both adapters with `fetch` stubbed and asserts the outbound request itself (model, URL, auth, and how the bytes reach the provider), the error classification that drives queue retries, and the Gemini structured-output parsing. `test/settings.test.ts` covers the settings service and `/api/settings` validation through the real router. Total **167 tests, up from 115**.
- [x] **The archive became public to read, admin-only to change.** Every read route uses `optionalAuth` (list, one item, playback URL, captions, caption status, search, suggestions); every write route keeps its own `requireAuth`. There is no `use('*', …)` left in the media/caption/search routers, so a new route declares an audience instead of inheriting one — and `optionalAuth` treats a missing or stale credential as "visitor" rather than a rejection, because a public read must not fail over an *optional* token.
  - **The audience narrows the payload, not the query.** A visitor's item has `captionErrorMessage` nulled and the caption job's attempts withheld: provider error text and retry bookkeeping are about the pipeline, not the content. `toPublicMediaItem()` is the single place that happens. Verified live — anonymous `GET /api/media` returns the item with `captionErrorMessage: null`, a signed playback URL serves real bytes (`206`), while `GET /api/media/quota` and every upload / patch / delete / generate / edit / delete-caption route return `401`.
  - The owner scope became a *parameter* rather than a session: admin reads stay `WHERE user_id = ?`, a visitor's read carries no owner predicate. One owner means the same rows either way, but the constraint stays real where it is given.
  - `test/security.test.ts` now holds **two** route lists — "every route that changes the archive refuses an unauthenticated request" and "every read route is reachable without a session" — so a route filed in the wrong group fails instead of passing quietly.
- [x] **Search merged into the home page.** `/search` and its nav tab are gone: it was the same grid, the same card and the same pagination as the library, differing only in what filled them. A query swaps the grid for results in place and stays linkable as `/library?q=…`. `SearchBar` owns the `?q=` parameter (debounced into the URL so typing doesn't stack a history entry per keystroke); `SearchResults` is a pure function of term and page, which is what keeps the back button and a shared link working.
- [x] **Visitors get the same shell and no controls.** `AppShell` wraps every route and the guard moved onto `/settings` and `/watch/:id/edit` instead of the shell — the old placement gated the entire app, which is precisely why a visitor could never reach the archive. Controls a viewer cannot use are absent rather than disabled, since a greyed-out button still advertises a capability they don't have. The quota meter became its own component that mounts only with a session: an anonymous `/media/quota` fetch is a guaranteed 401, which the API client reads as a dead session and would use to sign the owner out for nothing.
- [x] **Indexable, and honest about the limits.** `X-Robots-Tag: noindex, nofollow` → `all`, plus a `robots.txt` (allow `/`, disallow the owner's `/login` and `/settings`) and Open Graph / Twitter tags. No `og:url`, canonical or `sitemap.xml` on purpose: those need an absolute origin and the site is still on `archivedrop.pages.dev`, and a static sitemap cannot track items that live in Postgres. `useDocumentTitle` gives each route a real tab/history title; per-item metadata for crawlers and unfurlers would need prerendering or SSR, which this SPA does not do.
- [ ] Still uncovered: upload quota boundary cases; no DB-backed route integration tests (the suite fakes the database, so it cannot catch a bad SQL statement). **Newly worth watching now that reads are public:** nothing rate-limits the public read routes, and the playback endpoint issues a signed URL to anyone who asks for a known item id — fine for an open archive, but it is now the widest surface in the app.
...
- [x] **Groq model: `whisper-large-v3-turbo` on real audio.** A real 7.1 MB Arabic track transcribed on the first attempt into 32 timestamped segments with `language: Arabic` detected. The model id is no longer an env var — it is editable in Settings — so `whisper-large-v3` (more accurate, also translates) is one field away if accuracy disappoints on other material.
